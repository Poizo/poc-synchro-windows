...
type WindowDetails_X_Y = WindowDetails & {x: number, y: number};
...
function getScreenId() {
  const existingScreens = Object.keys(window.localStorage)
    ...
    .sort((a, b) => a - b);
  return (existingScreens.at(-1) || 0) + 1 || 1;
}
...
//   console.log('set screen details window detail =>', windowDetails);
}
...
function makeSVG() {
  ...
  screens
    ...
    .forEach(([key, screen], i) => {
      ...
      if (i === 0) {
        screenPath.moveTo((screen as WindowDetails_X_Y).x, (screen as WindowDetails_X_Y).y);
      } else {
        screenPath.lineTo((screen as WindowDetails_X_Y).x, (screen as WindowDetails_X_Y).y);
      }
      if (i === screens.length - 1) {
      screenPath.lineTo((screens[0][1] as WindowDetails_X_Y).x, (screens[0][1] as WindowDetails_X_Y).y);
      }
    });
...
function go() {
  ...
  timers.push(setInterval(setScreenDetails, 5000));
//   timers.push(setInterval(displayStats, 10));
  timers.push(setInterval(removeOld, 100));
  timers.push(setInterval(makeSVG, 5000));
}
